...
async function crawlDataGoat(productId) {
  ...
  try {
    ...
    $('div[data-qa="grid_cell_product"]').each((_i, el) => {
      ...
      gridCellName = $(el).attr('data-grid-cell-name');
      return false;
    });

    await page.close();
    await browser.close();
    const details = await extractDetailsFromProductGoat(gridCellName, productId);
    return details;
  } catch (err) {
...
async function extractDetailsFromProductGoat(url, productId) {
  ...
  try {
    ...
    await page.goto('https://www.goat.com', { waitUntil: 'networkidle2' });
    const response = await page.evaluate(async () => {
      const res = await fetch(`https://www.goat.com/web-api/v1/product_variants/buy_bar_data?productTemplateId=${url}&countryCode=VN`, {
        credentials: 'include',
        headers: {
          'Accept-Language':	'en-US,en;q=0.9',
          'Accept': 'application/json',
...
async function triggerAllSearchesFromAirtable() {
  try {
    ...
    for (const record of records) {
      ...
      const url = `https://${NGROK_STATIC_DOMAIN}/search?recordId=${encodeURIComponent(recordId)}&productId=${encodeURIComponent(productId)}&snkrdunkApi=${encodeURIComponent(snkrdunkApi)}&productType=${encodeURIComponent(productType)}`;
